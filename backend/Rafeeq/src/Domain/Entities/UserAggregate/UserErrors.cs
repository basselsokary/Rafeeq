...
public class UserErrors
{
    ...
    public static Error FavouriteNotFound(Guid key) =>
        Error.NotFound(
            "FAVOURITE_NOT_FOUND",
            $"Favourite with ID '{key}' was not found.");
    ...
    public static Error NationalityRequired => 
        Error.Validation(
            "USER_NATIONALITY_REQUIRED",
            "User nationality cannot be empty.");
    
    public static Error SiteAlreadyFavorite => 
        Error.Conflict(
            "SITE_ALREADY_FAVORITE",
            "Site is already in favorites.");
    
    public static Error DiscountRequired => 
        Error.Validation(
            "USER_DISCOUNT_REQUIRED",
