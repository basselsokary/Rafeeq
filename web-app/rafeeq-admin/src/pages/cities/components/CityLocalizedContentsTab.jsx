...
import { getLocalizedContents, addLocalizedContents, updateLocalizedContents } from '../../../api/citiesApi';
...
export default function CityLocalizedContentsTab({ cityId }) {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const res = await getLocalizedContents(cityId);
  ...
  const save = async () => {
    ...
    try {
      if (mode === 'add') {
        await addLocalizedContents(cityId, [form]);
        toast('Localized content added', 'success');
      } else {
        await updateLocalizedContents(cityId, [{ ...form, id: selected.id }]);
    ...
    <div>
      ...
      {loading ? <Spinner center /> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {contents.map((c) => (
            <div key={c.contentId || c.id} style={{
