...
import { getImages, uploadImages, deleteImages, setMainImage as setMainSiteImage } from '../../../api/sitesApi';
import Button from '../../../components/common/Button';
import Spinner from '../../../components/common/Spinner';
import ConfirmDialog from '../../../components/common/ConfirmDialog';
import { useToast } from '../../../components/common/Toast';

export default function ImagesTab({ siteId }) {
  ...
  const load = useCallback(async () => {
    try { setLoading(true);
      const res = await getImages(siteId);
  ...
  const handleSetMain = async (imageId) => {
    try {
      await setMainSiteImage(siteId, imageId);
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deleteImages(siteId, deleteIds);
  ...
  return (
    <div>
      ...
      >
        ...
        <div style={{ fontSize:16, fontWeight:600, color: 'var(--text)', marginBottom:6 }}>Upload Site Media</div>
        ...
        <div style={{ marginTop:14, fontSize:11, color: 'var(--text-muted)', display:'flex', alignItems:'center', justifyContent:'center', gap:10 }}>
          <span>JPG, PNG, WEBP</span><span style={{ width:3, height:3, borderRadius:'50%', background: 'var(--text-muted)', display:'inline-block' }} /><span>MAX 5MB PER FILE</span>
        </div>
      </div>
            ...
            >
              <label style={{ position:'absolute', top:8, left:8, zIndex:2 }}>
                <input
                  ...
                  style={{
                    width:16, height:16, accentColor:'var(--accent-btn)',
                    background:'rgba(11,15,26,.6)', borderRadius:4,
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:28 }}>
            <div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                {[['DIMENSIONS','—'],['FILE SIZE','—']].map(([label, fallback]) => (
                  <div key={label} style={{ background: 'var(--bg-base)', border:'1px solid var(--border)', borderRadius:8, padding:'10px 14px' }}>
                    ...
                    <div style={{ fontSize:14, fontWeight:600, color: 'var(--text)' }}>{fallback}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <ConfirmDialog
        ...
        message={deleteIds.length > 1 ? 'Permanently delete these images from the site?' : 'Permanently delete this image from the site?'}
        confirmLabel="Delete"
      />
    </div>
  );
