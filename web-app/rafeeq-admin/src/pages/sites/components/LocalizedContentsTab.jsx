...
import { getLocalizedContents, addLocalizedContents, updateLocalizedContents } from '../../../api/sitesApi';
...
export default function LocalizedContentsTab({ siteId }) {
  const toast = useToast();
  const [contents, setContents] = useState([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [selected, setSelected] = useState(null);
  const [form, setForm] = useState({ language: 'english', name: '', description: '', address: '', entryFeeNotes: '' });
  const [mode, setMode] = useState(null); // 'add' | 'edit'

  const load = async () => {
    try {
      setLoading(true);
      const res = await getLocalizedContents(siteId);
  ...
  const startAdd = () => {
    setForm({ language: 'english', name: '', description: '', address: '', entryFeeNotes: '' });
    setMode('add');
    setSelected(null);
  };

  const startEdit = (c) => {
    setForm({ language: c.language, name: c.name || '', description: c.description || '', address: c.address || '', entryFeeNotes: '' });
    setSelected(c);
    setMode('edit');
  };
  ...
  const save = async () => {
    ...
    try {
      if (mode === 'add') {
        await addLocalizedContents(siteId, [form]);
        toast('Localized content added', 'success');
      } else {
        await updateLocalizedContents(siteId, [{ ...form, id: selected.id }]);
        ...
        <div style={{
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              <input value={form.entryFeeNotes} onChange={(e) => setForm((f) => ({ ...f, entryFeeNotes: e.target.value }))} placeholder="Free entry note…" />
            </div>
          </div>
