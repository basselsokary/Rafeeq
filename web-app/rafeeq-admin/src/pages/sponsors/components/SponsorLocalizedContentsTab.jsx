...
import {
  addSponsorLocalizedContents,
  getSponsorLocalizedContents,
  updateSponsorLocalizedContents,
...
export default function SponsorLocalizedContentsTab({ sponsorId }) {
  const toast = useToast();
  const [contents, setContents] = useState([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [selected, setSelected] = useState(null);
  const [form, setForm] = useState({ language: 'english', title: '', description: '', address: '' });
  const [mode, setMode] = useState(null); // 'add' | 'edit'

  const load = async () => {
    try {
      setLoading(true);
      const res = await getSponsorLocalizedContents(sponsorId);
  ...
  const startAdd = () => {
    setForm({ language: 'english', title: '', description: '', address: '' });
    setMode('add');
    setSelected(null);
  };

  const startEdit = (c) => {
    setForm({ language: c.language, title: c.title || '', description: c.description || '', address: c.address || '' });
    setSelected(c);
    setMode('edit');
  };
  ...
  const save = async () => {
    ...
    try {
      if (mode === 'add') {
        await addSponsorLocalizedContents(sponsorId, [form]);
        toast('Localized content added', 'success');
      } else {
        await updateSponsorLocalizedContents(sponsorId, [{ ...form, id: selected.id }]);
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {contents.map((c) => (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, marginBottom: 2 }}>{c.title || '(no title)'}</div>
                {c.description && (
      ...
      {mode && (
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            <Button variant="secondary" size="sm" onClick={() => setMode(null)} disabled={saving}>
              Cancel
            </Button>
