...
import { getImages, uploadImages, deleteImages } from '../../../api/attractionsApi';
...
export default function AttractionImagesTab({ attractionId }) {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await getImages(attractionId);
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deleteImages(attractionId, deleteIds);
  ...
  return (
    <div>
      ...
      >
        ...
        <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--text)', marginBottom: 6 }}>Upload Attraction Media</div>
      ...
      <ConfirmDialog
        ...
        message={deleteIds.length > 1 ? 'Permanently delete these images from the attraction?' : 'Permanently delete this image from the attraction?'}
        confirmLabel="Delete"
      />
    </div>
  );
