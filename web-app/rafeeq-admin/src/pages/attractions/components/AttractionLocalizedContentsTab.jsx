...
import { getLocalizedContents, addLocalizedContents, updateLocalizedContents } from '../../../api/attractionsApi';
...
export default function AttractionLocalizedContentsTab({ attractionId }) {
  const toast = useToast();
  const [contents, setContents] = useState([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [selected, setSelected] = useState(null);
  const [form, setForm] = useState({ language: 'english', name: '', description: '', locationDescription: '' });
  const [mode, setMode] = useState(null);

  const load = async () => {
    try {
      setLoading(true);
      const res = await getLocalizedContents(attractionId);
  ...
  useEffect(() => { load(); }, [attractionId]);

  const startAdd = () => {
    setForm({ language: 'english', name: '', description: '', locationDescription: '' });
    setMode('add');
    setSelected(null);
  };
  ...
  const save = async () => {
    ...
    try {
      if (mode === 'add') {
        await addLocalizedContents(attractionId, [form]);
        toast('Localized content added', 'success');
      } else {
        await updateLocalizedContents(attractionId, [{ ...form, id: selected.id }]);
            ...
            <div key={c.id} style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                {c.locationDescription && (
                  <div style={{ color: 'var(--outline)', fontSize: 11, marginTop: 4 }}>
      ...
      {mode && (
        <div style={{
          ...
          <div className="form-group">
            ...
            <input value={form.locationDescription} onChange={(e) => setForm(f => ({ ...f, locationDescription: e.target.value }))} placeholder="e.g. North entrance of the complex" />
          </div>
