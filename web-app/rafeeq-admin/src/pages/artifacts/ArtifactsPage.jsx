...
import { getArtifactsBySiteId, createArtifact, getDashboardStats } from '../../api/artifactsApi';
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'relative', height: 200, overflow: 'hidden', background: 'var(--surface-container)' }}>
        {mainImage ? (
          <img
            src={mainImage} alt={artifact.name || ''}
...
export default function ArtifactsPage() {
  ...
  const loadArtifacts = useCallback(async (siteId, params) => {
    if (!siteId) {
      setArtifacts([]);
      setTotalCount(0);
      return;
    ...
    try {
      setLoading(true);
      const p = params || {};
      const res = await getArtifactsBySiteId(siteId, p);
      const d = res.data;
      const items = d?.data ?? d?.value?.data ?? d?.items ?? (Array.isArray(d) ? d : []);
      setArtifacts(items);
      setTotalCount(items.length);
    } catch { setArtifacts([]); setTotalCount(0); }
  ...
  useEffect(() => {
    ...
    loadArtifacts(selectedSiteId, buildParams());
  }, [selectedSiteId]);
  useEffect(() => { loadStats(); }, []);
  ...
  const handleCreate = async (payload) => {
    ...
    try {
      ...
      newId ? navigate(`/artifacts/${newId}`) : loadArtifacts(selectedSiteId, buildParams());
    } catch (e) {
  ...
  const assignedToCount = stats.assignedToSites;
        ...
        <span>Artifacts Management</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 16, marginBottom: 28 }}>
            <StatCard
              ...
              sub={<span>{assignedToCount} assigned to sites</span>}
            ...
            />
            <StatCard
              label="Artifacts Assigned to Sites" value={assignedToCount} loading={statsLoading}
          ...
          {!selectedSiteId ? (
            <div style={{ padding: '80px 20px', textAlign: 'center', color: 'var(--outline)', fontSize: 14 }}>
              Select a site to view its artifacts.
            </div>
          ) : loading ? (
